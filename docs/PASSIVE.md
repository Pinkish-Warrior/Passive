...
The goal is to build a **passive OSINT (Open Source Intelligence) CLI tool** that takes a person's identity input and automatically gathers publicly available information about them — without directly interacting with the target system.

Three input types are supported:

- **Full name** — looks up phone number and address from public directories
- **IP address** — retrieves the city and ISP of the IP
- **Username** — checks if the username exists across 5+ social networks

All results are saved to a sequentially named output file (`result.txt`, `result2.txt`, etc.).

The project is educational — aimed at building familiarity with OSINT methods and open source investigative techniques.
